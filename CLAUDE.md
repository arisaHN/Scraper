...
- `tests/test_primor_scraper.py` — pure unit tests for `_reviews_url()` construction and `ReviewNormalizer.from_primor()` (always run, no network — including a stability check that the sha1-synthesized `external_review_id` is deterministic for identical input and distinguishes different reviews), plus live integration tests against the real it.primor.eu sitemap/product pages and the `reviews.primor.eu` JSON payload (discovery, including an Armani case that specifically exercises the substring-not-prefix slug matching; review fetch; self-deriving `since`-cutoff); live tests skipped automatically unless `PRIMOR_ENABLED` is set. No browser/credentials needed — the site has no bot gating anywhere.
